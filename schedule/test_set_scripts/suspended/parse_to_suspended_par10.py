...
from calculate_suspended_par10 import par10

result_mapping = {'unknown': 2, 'solved': 1, 'unsolvable': 0}


def parse_to_par10(data_dir, order_file, timeout_file, seed_lb, seed_ub,
                   seconds_multiplier):
    data_files = []
    file_extension = ".csv"
    ...
    par10_score = par10(schedule_filenames, schedule_timeouts, seed_lb,
                        seed_ub)
...
if __name__ == "__main__":
    if len(argv) < 7:
        ...
        score = parse_to_par10(argv[1], argv[2], argv[3], int(argv[4]),
